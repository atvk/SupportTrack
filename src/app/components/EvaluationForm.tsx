...
export default function EvaluationForm() {
  ...
  const calculateTotalScore = () => {
    // Проверка на риски: если в критерии risks выбрана ошибка (не "ok"), то общая оценка 0
  ...
  return (
    ...
    >
      ...
      {/* Основные поля (автоматические + ручные) */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="flex flex-wrap gap-6">
          ...
          <div className="relative flex items-center justify-center w-16 h-16 rounded-full bg-indigo-100 dark:bg-indigo-900/60 text-indigo-500 dark:text-indigo-300 transition-transform hover:scale-105 cursor-default overflow-visible">
            <CalendarBlankIcon 
              size={28} 
              weight="light" 
              className="absolute opacity-30" 
              style={{ top: '50%', left: '50%', transform: 'translate(-50%, -50%)' }} 
            />
            <div className="absolute top-0 right-0 transform translate-x-1/2 -translate-y-1/2 bg-white dark:bg-gray-800 text-indigo-600 dark:text-indigo-300 text-xs font-bold rounded-md px-2 py-1 shadow-lg border border-indigo-200 dark:border-indigo-700 whitespace-nowrap">
              <div>Week {formData.week}</div>
              <div>{formatDate(formData.date)}</div>
            </div>
          </div>
        ...
        {/* Контакт (ссылка) - ручной ввод */}
        <div className="md:col-span-2">
          <label
            htmlFor="contact"
            className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2"
          >
            Контакт (ссылка на тикет)
          </label>
          <input
            type="url"
            id="contact"
      ...
      {/* Критерии с select (datalist) - без отображения веса */}
